...
class AudioTransientSplitter:
    ...
    def detect_transient_segments(self, y: np.ndarray) -> List[Tuple[int, int]]:
        """
        ...
        for i, start in enumerate(onset_samples):
            ...
            if len(end_candidates) > 0:
                ...
                # Ensure end doesn't go beyond next onset
                if i < len(onset_samples) - 1 and end > onset_samples[i + 1]:
                    end = onset_samples[i + 1]
            else:
                # If no drop below threshold, use next onset or end of audio
                if i < len(onset_samples) - 1:
                    end = onset_samples[i + 1]
                else:
                    end = len(y) - 1
    ...
    def split_audio_by_segments(self, 
        ...
        # Detect transient segments (start and end points)
        segments = self.detect_transient_segments(y_mono)
    ...
    def visualize_transient_segments(self, file_path: str, output_file: Optional[str] = None, pre_offset_ms: float = 0):
        ...
        segments = self.detect_transient_segments(y)
        ...
        # Add legend
        legend_items = ['Waveform', 'Segment']
        if pre_offset_ms > 0 and segments:
            legend_items.append('Pre-roll')
